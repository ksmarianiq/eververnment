...
@section('Evenement')
    ...
    <div class="container mb-3 ">
        <div class="card">
            <div class="card-header" style="background-color:#0b3544;">
                <h2 class="card-title text-white fw-bolder">Informations Supplémentaires</h2>
            ...
            <div class="card-body">
                <a id="btnModalFormOrganisteur" href="#modalFormOrganisteur" class="btn text-white mb-4"
                    style="background-color:#0b3544;" data-toggle="modal" data-backdrop="static" data-keyboard="false"><i
                        class="fas fa-plus-circle"></i> <span>Ajouter une Information</span></a>
    ...
    <!-- Modal-->
    @include('Admin.pages.File_Information.deleteInformation')
    @include('Admin.pages.File_Information.editInformation')
    @include('Admin.pages.File_Information.addInformation')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.editbtn', function() {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ route('Information.edit', ':id') }}".replace(':id', id),
                    success: function(response) {
                        console.log(response);
                        $('#titre').val(response.Information.titre);
                        $('#evn_id').val(response.Information.evn_id);
                        $('#codeInf').val(response.Information.codeInf);
                        $('#id').val(response.Information.id);
                    }
                })
            });
        });
